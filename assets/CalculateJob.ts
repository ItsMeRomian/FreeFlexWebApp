...
export default class CalculateJob {
  ...
  constructor(job: JobInterface) {
    ...
    this.job = {
      ...
      car_id: job.car_id,
      factoring_percentage: job.factoring_percentage,
      client_id: job.client_id,
      date: job.date,
      facto_nr: job.facto_nr,
      payed: job.payed,
      btw_percentage: job.btw_percentage,
      id: job.id,
      daysToPay: job.daysToPay,
      rating: job.rating,
      checkedOut: job.checkedOut,
    };
    this.workedHours = 0;
    this.setWorkedHours();
    ...
    console.log("Calculating new job:,", this.job);
  }
  setWorkedHours() {
    const start = this.timeToFloat(this.job.start);
    let end = this.timeToFloat(this.job.end);
    let pauze = this.timeToFloat(this.job.break);
    if (end < start) {
      end = end + 24;
    }
  ...
  setFactoringExclBTW() {
    this.factoringExclBTW = this.getInclBTW() * (this.job.factoring_percentage / 100);
  }
  setFactoringBTW() {
  ...
  isFactoring(): boolean {
    if (this.job.factoring_percentage === 0) {
      return true;
    } else {
      return false;
    }
  ...
  getJobStatus() {
    ...
    } else if (this.job.payed || this.job.checkedOut) {
      return 0;
    } else {
      return 4;
    }
